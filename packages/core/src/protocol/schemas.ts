...
export const DailyExperienceReportSchema = z.object({
  ...
  experience_coverage: z.object({
    ...
    total_lessons: z.number().int().min(0).default(0),
    total_wiki_evidence: z.number().int().min(0).default(0),
    pending_curation: z.number().int().min(0).default(0),
    privacy_blocked: z.number().int().min(0).default(0),
    low_signal_rejected: z.number().int().min(0).default(0),
    items: z.array(z.object({
      source_id: z.string().min(1),
      ...
      lesson_claims: z.array(z.string().min(1)).default([]),
      ...
      status: z.enum(["raw_only", "needs_curation", "privacy_blocked", "low_signal_rejected", "lesson_only", "wiki_evidence", "proposal", "stable_kb"]),
  ...
  lessons: z.object({
	    ...
	    authority_contract: z.object({
	      wiki_semantic_input: z.enum(["lesson_clusters", "none"]).default("none"),
	      context_rank: z.array(z.enum([
	        "stable_pb_page",
	        "promoted_skill",
	      ...
	      promotion_evidence: z.object({
	        lesson_state_authority: z.boolean().default(false),
	        legacy_distilled: z.literal(false).default(false),
	        gbrain_sidecar: z.literal(false).default(false),
	        agentmemory_sidecar: z.literal(false).default(false),
	      }).default({
  ...
  personal_ga: z.object({
    type: z.literal("personal_ga_report"),
    mode: z.enum(["production_ai", "degraded_no_ai", "budget_exhausted"]),
    source_coverage: z.array(z.object({
      agent: z.string().min(1),
      source_kind: z.string().min(1),
      configured: z.boolean(),
      available: z.boolean(),
      items: z.number().int().min(0),
      content_spans: z.number().int().min(0).optional(),
      origin: z.string().optional(),
      trust: z.string().optional(),
      privacy_scope: z.string().optional(),
      blocking: z.boolean().optional(),
    })),
    lesson_count: z.number().int().min(0),
    disposition_count: z.number().int().min(0),
    golden_validation: z.object({
      matched: z.number().int().min(0),
      required: z.number().int().min(0),
      missed: z.array(z.string()),
    }),
    leakage_scan: z.object({
      passed: z.boolean(),
    ...
    agent_consumption: z.array(z.object({
      surface: z.string().min(1),
      available: z.boolean(),
      authority: z.array(z.string()),
    })),
    queue: z.object({
      type: z.literal("personal_queue_report"),
      run_kind: z.enum(["full", "bounded_smoke"]),
      full_run: z.boolean(),
      bounded_smoke: z.boolean(),
      planned_source_items: z.number().int().min(0),
      selected_spans: z.number().int().min(0),
      processed_spans: z.number().int().min(0),
      cache_hits: z.number().int().min(0),
      uncached_ai_calls: z.number().int().min(0),
      skipped_low_priority_items: z.number().int().min(0),
      remaining_high_priority_items: z.number().int().min(0),
      resume_state: z.enum(["complete", "resumable", "blocked"]),
      high_priority_sources: z.array(z.object({
        role: z.enum(["local_openclaw", "trusted_remote_openclaw", "codex_app", "codex_cliproxyapi"]),
        source_name: z.string().min(1),
        agent: z.string().min(1),
        configured: z.boolean(),
        planned_items: z.number().int().min(0),
    ...
    dispositions: z.array(z.object({
      ...
      privacy_tier: z.string().min(1),
      portability: z.string().min(1),
      applies_to_agents: z.array(z.string()),
      applies_to_systems: z.array(z.string()),
    })),
    production_ready: z.boolean(),
    blocking_reasons: z.array(z.string()),
    warnings: z.array(z.string()).default([]),
