...
当前要做的是把 PraxisBase 从 OpenClaw/K8s 场景扩展成 **个人与团队多 agent 的经验层**。第一版只做 CLI/file protocol、adapter profile、capture/outbox、raw vault、distill proposal 和 context retrieval 的基础能力；不要做深插件、GUI、向量库、中心数据库或完整平台。
...
5. 按 M0 到 M4 分批实现，每批必须跑对应测试和 `pnpm check`。

硬性禁区：

- 不把 raw logs、完整 transcripts、飞书原文、tokens、cookies 或密钥写入 Git。
- 不让 capture/watch/distill 直接修改 `kb/` 或 `skills/`。
...
团队用户有 OpenClaw 沙箱、K8s 故障定位系统、飞书机器人、CI job、Hermes curator、临时修复 agent。
...
稳定知识仍然只通过 `review` 和 `promote` 进入 `kb/` 或 `skills/`。`capture`、`watch` 和 `distill` 只能写 outbox、inbox、reports、runs、exceptions。
...
每个里程碑都必须有对应 BDD 场景、CLI smoke test 和 docs update。M0 到 M2 可以先只支持 Codex、OpenClaw 和 generic profile；Claude Code、OpenCode、Hermes profile 可以在 M1 完成，但深度插件不进入 MVP。
