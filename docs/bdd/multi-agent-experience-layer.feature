...
功能: Multi-Agent Experience Layer
  ...
  作为 PraxisBase 的维护者
  我需要统一的 CLI/file protocol 来导入原生记忆、获取上下文、捕获经验、安装轻量适配、生成 proposal，并保持稳定知识可审计

  背景:
    假如 当前工作目录是一个已经初始化的 PraxisBase 知识仓库
    并且 系统使用 protocol_version "0.1"
    并且 memory import、memory refresh、capture、watch、context 和 distill 命令不能直接修改 "kb/" 或 "skills/"
    并且 raw transcripts、完整日志、飞书原文、tokens、cookies 和密钥不能写入 Git
    并且 agent 原生记忆不是无审核的双向实时同步来源
  ...
  场景: M3 Hermes 原生 skill summary 只进入 proposal 候选
    假如 一个 Hermes native memory source descriptor 包含 kind "skill_summary"、source_ref "raw-vault://hermes/skill-auth-repair"、source_hash "sha256:hermes1" 和 redacted_summary
    当 用户运行 "praxisbase memory import --agent hermes --source hermes-skill-summary.json --json"
    那么 系统写入 ".praxisbase/reports/memory/<run-id>.json"
    并且 输出包含 proposal candidate
    并且 proposal candidate 保留 source_ref 和 source_hash
    并且 report 中 "changed_stable_knowledge" 是 false
    并且 系统不把 Hermes skill summary 原文写入 "skills/"

  场景: M3 OpenHuman preference 默认保持 personal scope
    假如 一个 OpenHuman native memory source descriptor 包含 kind "preference" 和 redacted_summary "Prefer terse implementation plans."
    当 用户运行 "praxisbase memory import --agent openhuman --source openhuman-preference.json --json"
    那么 输出的 proposal candidate scope 是 "personal"
    并且 系统不会自动提升为 "project"、"team" 或 "org"
    并且 系统不写入 "kb/" 或 "skills/"
  ...
  场景: M4 diagnosis context 遵守预算并保留 citations
  ...
  场景: M4 context unavailable 不阻塞 agent
  ...
  场景: M5 distill run 生成 proposal 但不改稳定知识
  ...
  场景: M5 distill 默认 personal scope
  ...
  场景: M5 watch once 没有可监听路径时降级为 warning
  ...
  场景: M6 smoke flow 可证明最小闭环
    当 用户依次运行 init、install dry-run、memory import、capture finish、distill run、context get
    那么 每个命令都返回 machine-readable JSON
    并且 memory import report 出现在 ".praxisbase/reports/memory/"
    并且 capture 出现在 ".praxisbase/outbox/captures/"
    并且 distill report 出现在 ".praxisbase/reports/distill/"
    并且 context response 包含 stage、warnings 和 citations
    并且 memory import、memory refresh、capture、watch、distill 都没有修改稳定知识
