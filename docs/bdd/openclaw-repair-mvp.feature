...
功能: OpenClaw Repair MVP agent knowledge substrate
  ...
  背景:
    假如 当前工作目录是一个空的 LLMHTML 测试仓库
    并且 系统使用 protocol_version "0.1"
    并且 没有外部搜索服务、向量数据库、MCP server 或 Hermes runner

  场景: 初始化 OpenClaw 修复知识底座
    当 用户运行 "llmhtml init"
    那么 系统创建 ".llmhtml/config.yaml"
    并且 系统创建 ".llmhtml/policies/autonomy.yaml"
    并且 系统创建 ".llmhtml/policies/risk-rules.yaml"
    并且 系统创建 ".llmhtml/inbox/episodes"
    并且 系统创建 ".llmhtml/inbox/proposals"
    并且 系统创建 ".llmhtml/outbox/episodes"
    ...
    并且 ".llmhtml/config.yaml" 包含 "protocol_version: \"0.1\""
  ...
  场景: 临时修复 agent 从 auth expired 日志获取 repair context
    ...
    当 agent 运行 "llmhtml repair-context openclaw --logs auth-expired.log --json"
  ...
  场景: 临时修复 agent 提交成功修复 episode
    ...
    当 agent 运行 "llmhtml episode submit episode.json"
    那么 系统写入 ".llmhtml/inbox/episodes/<episode-id>.json"
  ...
  场景: 缺少 provenance 的 episode 被拒绝
    ...
    当 agent 运行 "llmhtml episode submit episode-without-source.json"
    那么 命令失败
  ...
  场景: agent 提交带证据的 known fix proposal
    ...
    当 agent 运行 "llmhtml propose known-fix-proposal.json"
    那么 系统写入 ".llmhtml/inbox/proposals/<proposal-id>.json"
    并且 写入文件保留 evidence
  ...
  场景: 缺少 evidence hash 的 proposal 被拒绝
    ...
    当 agent 运行 "llmhtml propose proposal-without-hash.json"
    那么 命令失败
  ...
  场景: 中风险 known fix proposal 由 AI reviewer 自动批准
    ...
    当 scheduled job 运行 "llmhtml review --auto"
    那么 系统写入 ".llmhtml/inbox/reviews/<review-id>.json"
  ...
  场景: 高风险 policy proposal 进入人工异常队列
    ...
    当 scheduled job 运行 "llmhtml promote --auto"
    那么 系统不修改 "kb/"
    并且 系统不修改 "skills/"
  ...
  场景: 生成静态 repair bundle 和索引
    假如 工作区已经运行 "llmhtml init"
    当 用户运行 "llmhtml build"
  ...
  场景: 最新 bundle 不可用时 agent 使用 last-known-good
    ...
    并且 最新 bundle 下载失败
    当 agent 运行 "llmhtml bundle fetch openclaw --signature openclaw:claude-auth-expired"
  ...
  场景: 知识仓库不可用时 agent 写入 outbox
    ...
    当 agent 运行 "llmhtml episode submit episode.json --offline-ok"
    那么 系统写入 ".llmhtml/outbox/episodes/<episode-id>.json"
  ...
  场景: GitLab scheduled pipeline 串行化写任务
    ...
    那么 "llmhtml:review" job 包含 "resource_group: llmhtml-write"
    并且 "llmhtml:promote" job 包含 "resource_group: llmhtml-write"
